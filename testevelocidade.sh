...
def shell():
    ...
    printer('Testando Velocidade %(isp)s (%(ip)s)...' % speedtest.config['client'],
            quiet)

    if not args.mini:
        printer('Procurando Melhor Servidor para o teste...', quiet)
        try:
    ...
    printer('Servidor by %(sponsor)s (%(name)s) [%(d)0.2f km]: '
            '%(latency)s ms' % results.server, quiet)

    if args.download:
        printer('Testando Download', quiet,
                end=('', '\n')[bool(debug)])
        speedtest.download(callback=callback)
        printer('Download: %0.2f M%s/s' %
                ((results.download / 100.0 / 1000.0) / args.units[1],
                 args.units[0]),
                quiet)
    else:
